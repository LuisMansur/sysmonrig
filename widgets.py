...
import json, os, sys, time, threading, urllib.request, math
from ctypes import windll, byref, c_int, c_long, c_ulong, WINFUNCTYPE, POINTER
from ctypes.wintypes import HWND, UINT, WPARAM, LPARAM, RECT, MSG
...
F_GIANT = _f('arialbd.ttf', 80)
F_BIG   = _f('arialbd.ttf', 42)
F_MED   = _f('arialbd.ttf', 28)
F_BODY  = _f('arial.ttf',   24)
F_SMALL = _f('arial.ttf',   20)
F_LABEL = _f('arialbd.ttf', 17)
...
HTCAPTION=2; WM_NCHITTEST=0x0084; WM_LBUTTONUP=0x0202
WM_RBUTTONDOWN=0x0204; WM_DESTROY=0x0002
...
def _register_class():
    global _wndproc_cb
    _wndproc_cb=WNDPROC(_wnd_proc)
    hinstance=kernel32.GetModuleHandleW(None)
    wc=WNDCLASSEX()
    wc.cbSize=ctypes.sizeof(WNDCLASSEX); wc.style=0
...
class Widget:
    def __init__(self,wid,W,H,xy):
        global _hinstance
        self.id=wid; self.W=W; self.H=H; self._dragging=False
    ...
    def _draw(self,draw,stats): pass

# ── Widget drawings ───────────────────────────────────────────────────────────

class CpuWidget(Widget):
    def _draw(self, draw, s):
        W,H     = self.W, self.H
        cpu     = s.get('cpu', {})
        u       = cpu.get('usage', 0);    col = uc(u)
        ct      = cpu.get('temp')
        freq    = cpu.get('freq_ghz', '--')
        cores   = cpu.get('cores', '--')
        threads = cpu.get('threads', '--')

        ph=58; pad=10
        r  = int((H-ph-pad*3)*0.46)
        rw = max(14, int(r*0.14))
        cx = W//2; cy = r+pad+rw
        ...
        # Temp ring — bottom-left of main ring, contained within it
        if ct:
            tr   = int(r*0.28)
            trx  = cx - r + tr + 2
            try_ = cy + r - tr - 2
        ...
        tx(draw,hw//2+2,        py+34,f'{cores} / {threads}', BLUE,F_BODY)
        tx(draw,hw+(W-hw)//2+2, py+10,'FREQUENCY',            DIM, F_LABEL)
        tx(draw,hw+(W-hw)//2+2, py+34,f'{freq} GHz',          BLUE,F_BODY)


class GpuWidget(Widget):
    # Change to your GPU's boost clock in MHz (check GPU-Z or nvidia-smi)
    GPU_MAX_CLK = 2800

    def _draw(self, draw, s):
        W,H  = self.W, self.H
        gpu  = s.get('gpu',{}); u=gpu.get('gpu_usage',0); col=uc(u)
        gt   = gpu.get('temp',0)
        vp   = (gpu.get('mem_used',0)/max(gpu.get('mem_total',1),1))*100
        fan  = gpu.get('fan_speed',0)
        pwr  = gpu.get('power_draw',0)
        ...
        if name:
            tx(draw,cx,cy+46,name[:18],DIM,F_LABEL)

        # Temp ring — bottom-left of main ring
        tr   = int(r*0.28)
        trx  = cx-r+tr+2
        try_ = cy+r-tr-2
...
class RamWidget(Widget):
    def _draw(self, draw, s):
        W,H   = self.W, self.H
        ram   = s.get('ram',{}); p=ram.get('percent',0); col=uc(p)
        ...
        tx(draw,hw//2+2,        py+34,f'{used} GB', col, F_BODY)
...
class NetWidget(Widget):
    ...
    def _draw(self, draw, s):
        W,H = self.W, self.H
        net = s.get('network',{})
        up  = net.get('upload_mbps',0)
        dn  = net.get('download_mbps',0)
        ...
        half = H//2
        r    = int(half*0.38); rw = max(10,int(r*0.18))

        # Rings LEFT-aligned, peak info on the RIGHT
        cx  = r + rw + 10          # ring centre x
        px  = cx + r + rw + 18    # x where peak text starts (left anchor)

        # ── Upload ──
        cyu = half//2
        draw_ring(draw,cx,cyu,r,min(up/scale*100,100),GREEN,width=rw)
        tx(draw,cx,cyu-18,f'{up}',   GREEN,F_BIG)
        tx(draw,cx,cyu+14,'Mbps',    DIM,  F_SMALL)
        tx(draw,cx,cyu+34,'UPLOAD',  DIM,  F_LABEL)
        # Peak — right of ring, larger text
        tx(draw,px,cyu-16,'PEAK',                         DIM,  F_LABEL,'la')
        tx(draw,px,cyu+8, f'{NetWidget._peak_up} Mbps',  GREEN,F_BODY, 'la')
...
if __name__ == '__main__':
    ...
    CpuWidget('cpu',W4,H,(xs['cpu'],wy))
    GpuWidget('gpu',W4,H,(xs['gpu'],wy))
    RamWidget('ram',W4,H,(xs['ram'],wy))
    NetWidget('net',W4,H,(xs['net'],wy))
